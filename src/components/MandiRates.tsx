      ...
      <CardContent>
        <div className="space-y-4">
          {filteredCommodities.map((commodity, index) => (
            <div key={index} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-muted/40 hover:bg-muted/70 rounded-lg border gap-3 transition-colors">
              <div>
                <h4 className="font-semibold text-foreground text-sm">{translateItemName(commodity.name, language, 'commodity')}</h4>
              ...
              <div className="flex items-center justify-between sm:justify-end gap-3">
                <div className="text-left sm:text-right">
                  <div className="flex items-center gap-1.5 font-bold text-foreground">
                    <span>₹{commodity.modalPrice}</span>
                    <span className="text-xs font-normal text-muted-foreground">/Qtl</span>
                    {getTrendIcon(commodity.trend)}
                  </div>
                  <div className={`text-xs ${getTrendColor(commodity.trend)} font-medium`}>
                    {commodity.change !== 0 && (
                      <>
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() => {
                    toast.success(`Loaded ${commodity.name} into Net Revenue Spatial Calculator!`);
                    window.scrollTo({ top: 400, behavior: "smooth" });
                  }}
                  className="h-7 text-xs border-emerald-300 text-emerald-800 hover:bg-emerald-50 dark:text-emerald-300 flex items-center gap-1"
                  aria-label={`Calculate net profit for ${commodity.name}`}
                >
